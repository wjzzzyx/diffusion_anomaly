...
class GaussianDiffusion(nn.Module):
    ...
    def calc_bpd_loop(self, x_start, clip_denoised=True, model_kwargs=None):
        ...
        for t in list(range(self.num_timesteps)):
            ...
            xstart_mse.append(torch.mean((out["pred_xstart"] - x_start) ** 2, dim=(1, 2, 3)))

        vb = torch.stack(vb, dim=1)
        pred_xstart = torch.stack(pred_xstart, dim=1)
    ...
    def _vb_terms_bpd(
        self, x_start, x_t, t, clip_denoised=True
    ):
        """
        ...
        p_mean_x_tm1, p_var_x_tm1, p_logvar_x_tm1, p_x_start = self.p_mean_variance(
            x_t, t, clip_denoised=clip_denoised
        )
        kl = normal_kl(
            true_mean, true_log_variance_clipped, p_mean_x_tm1, p_logvar_x_tm1
        )
        kl = kl / math.log(2.0)
        # kl = torch.mean(kl, dim=(1, 2, 3)) / math.log(2.0)

        decoder_nll = -discretized_gaussian_log_likelihood(
            x_start, means=p_mean_x_tm1, log_scales=0.5 * p_logvar_x_tm1
        )
        assert decoder_nll.shape == x_start.shape
        decoder_nll = decoder_nll / math.log(2.0)
        # decoder_nll = torch.mean(decoder_nll, dim=(1, 2, 3)) / math.log(2.0)

        # At the first timestep return the decoder NLL,
        # otherwise return KL(q(x_{t-1}|x_t,x_0) || p(x_{t-1}|x_t))
        output = torch.where((t == 0)[:, None, None, None], decoder_nll, kl)
        return {"output": output, "pred_xstart": p_x_start}
    ...
    def _prior_bpd(self, x_start):
        ...
        kl_prior = kl_prior / math.log(2.0)
        # torch.mean(kl_prior, dim=(1, 2, 3)) / math.log(2.0)
        return kl_prior
...
class Trainer():
    ...
    def __init__(self, config):
        ...
        self.ema = ema_pytorch.EMA(self.diffusion, beta=0.995, update_every=10)
    ...
    def train_step(self, feeddict):
        self.optimizer.zero_grad()
        loss = self.diffusion(feeddict['img'])
        loss.backward()
    ...
    def val_step(self, n: int):
        self.ema.ema_model.eval()
        samples = self.ema.ema_model.sample(batch_size=n)
        return samples
